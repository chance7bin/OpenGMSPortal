  ...
  methods: {
    ...
    getGraphData(year) {
      ...
      $.ajax({
        ...
        type: "get",
        data: { type: "model", text: "swmm", sTime: year, eTime: year },
    ...
    createHistoryTrendGraph() {
      ...
      let that = this;
      $.ajax({
        url: "/static/KnowledgeGraph/assets/swmm/researchTrend.json",
        type: "get",
        dataType: "json",
    ...
    createHistoryCountryGraph() {
      ...
      if (this.worldData) {
        this.$nextTick(() => {
          ...
          $.ajax({
            url: "/static/KnowledgeGraph/assets/swmm/researchCountry.json",
            type: "get",
            dataType: "json",
    ...
    prepareLeafletMap() {
      ...
      if (this.locationOfModel.length === 0) {
        $.ajax({
          url: "/static/KnowledgeGraph/assets/swmm/locationOfModel.json",
          type: "get",
      ...
      this.$nextTick(() => {
        if (this.keywordChart === null) {
          $.ajax({
            ...
            data: { id: "064a849c-daec-47a7-97b9-ddd323a859b0" },
            type: "get",
            success: function(data) {
                let loadingMask = document.getElementById('loadingDiv');
                loadingMask.parentNode.removeChild(loadingMask);//当数据获取之后，清除掉数据加载div
    ...
    createD3StreamGraph(data) {
      ...
      let y = d3
        .scaleLinear()
        .domain([-legendCount[0].count*2, legendCount[0].count*2])
        .range([height, 0]);
