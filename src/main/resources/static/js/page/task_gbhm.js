var vue = new Vue({
    ...
    methods: {
        ...
        //2021暑期
        //2021暑期
        jointFileType(fileType){
            let result='';
            if(fileType!=undefined) {
                for (let i = 0; i < fileType.length; ++i) {
                    result += fileType[i].toUpperCase();
        ...
        setDateValue(index,target,value){
            console.log(value)
            let eventId = target.eventId
            let eventName = target.eventName

            let curState = this.info.modelInfo.states[index];
            let curEvent = this.findEventByEventID(curState,eventId)
            for(let k=0;k<curEvent.children.length;k++){
                if(curEvent.children[k].eventName==eventName){
                    this.$set(curEvent.children[k],"value",value)
                    this.$forceUpdate()
                }
            }

        },
        setInputValue(index,target,value){
            let eventId = target.eventId
            let eventName = target.eventName
            ...
            for(let k=0;k<curEvent.children.length;k++){
                if(curEvent.children[k].eventName==eventName){
                    ...
                    this.$forceUpdate()
                    this.$nextTick(()=>{
                        this.$refs['input'+eventId][0].focus()
        ...
        setSelectValue(index,target,value){
            let eventId = target.eventId
            let eventName = target.eventName
            ...
            let configInfo_data = target.configInfo.data;
            if(configInfo_data.length!=0){
                for(let i = 0;i<configInfo_data.length;++i){
                    if(configInfo_data[i].value == value){
                        let curData = configInfo_data[i]
                        let relatedChanges = curData.relatedChanges;
                        for(let j = 0;j<relatedChanges.length;++j){
                            let re_state = this.findState(relatedChanges[j].stateID);
                            let re_event = this.findEventByEventName(re_state,relatedChanges[j].eventName)
                            re_event.eventDesc = relatedChanges[j].description;}
                        }
                    }
                }
        },
        findConfig(datasetItemName) {
            let pageConfig = this.info.modelInfo.pageConfig;
            for(let i = 0; i<pageConfig.length;++i){
                if(pageConfig[i].datasetItemName === datasetItemName)
                    return pageConfig[i].nodes;
            }
        },
        findConfigItem() {
                ...
                {
                    for(let j = 0;j< states[i].event.length;++j)
                    {
                        states[i].event[j].data[0].configNodes = this.findConfig(states[i].event[j].data[0].text);
                    }
                }
                console.log(this.info.modelInfo)
        },
        findConfigNode(datasetItemName,nodesName) {
            for(let i = 0; i<this.info.modelInfo.pageConfig.length;++i)
            {
                let pageConfig = this.info.modelInfo.pageConfig;
                if(pageConfig[i].datasetItemName == datasetItemName)
                {
                    for(let j = 0;j <pageConfig[i].nodes.length;++j)
                    {
                        if (pageConfig[i].nodes[j].name == nodesName)
                            return pageConfig[i].nodes[j];
                    }
                }
            }
        },
        findFaterEvent(eventId){
            //在states中找event
            for(let i = 0; i< this.info.modelInfo.states.length;++i)
            {
                let curState = this.info.modelInfo.states[i];
                for(let j = 0; j<curState.event.length;++j)
                {
                    let curEvent = curState.event[j];
                    if(curEvent.eventId == eventId)
                        return curEvent
                }
            }
        },
        findEventByEventName(state,eventName){
        //    在state中找event
            for(let i = 0;i<state.event.length;++i){
                if(state.event[i].eventName == eventName){
                    return state.event[i];
                }
            }
        },
        findEventByEventID(state,eventId){
            //    在state中找event
            for(let i = 0;i<state.event.length;++i){
                if(state.event[i].eventId == eventId){
                    return state.event[i];
                }
            }
        },
        ...
        findState(stateID){
            for(let i = 0;i<this.info.modelInfo.states.length;++i){
                if(this.info.modelInfo.states[i].Id == stateID){
                    return this.info.modelInfo.states[i]
                }
            }
        },
        loadTestDataInput(eventId) {
            let curEvent = this.findFaterEvent(eventId);
            if(curEvent.tag!=undefined){
                curEvent.value = curEvent.url;
                console.log(this.info.modelInfo.states[0])
                return curEvent.tag+'.'+curEvent.suffix;
            }
            else
                return '';
        },
        ...
        uploadToDataContainer(file, event) {
            ...
            $.get("/dataManager/dataContainerIpAndPort", (result) => {
                ...
                $.ajax({
                    type: "post",
                    url: "/dataTransferServer/configData",
                    data: formData,
                    async: true,
                    processData: false,
                    contentType: false,
                    ...
                    if (event.eventType == "response" && event.children != undefined) {
                        ...
                        for (k = 0; k < children.length; k++) {
                            ...
                            if (child.value == undefined || (child.value!=undefined&&child.value.toString().trim() == '')) {
                                ...
                                content += "<XDO name=\"" + child.eventName + "\" kernelType=\"" + child.dataType.toLowerCase() + "\" value=\"" + child.value + "\" /> ";
                            }
                        }
                        if (content === "") {
                            continue;
        ...
        selectDataFromPersonal() {
            if (this.downloadDataSetName[0]!=undefined) {
                //选了
                let tag = 0;
                    if(this.eventChoosing.configInfo.constraint!=undefined&&this.eventChoosing.configInfo.constraint.fileType!=undefined)
                    {
                        let fileType =  this.eventChoosing.configInfo.constraint.fileType;
                        for(let i = 0; i < fileType.length;++i)
                        {
                            if(fileType[i] == this.downloadDataSetName[0].suffix){
                                tag = 1;
                                break;
                            }
                        }
                    ...
                    {
                        let curEvent = this.findFaterEvent(this.eventChoosing.eventId)
                        this.showDataChose = false;
                        this.$set(curEvent,"tag",this.downloadDataSetName[0].name)
                        this.$set(curEvent,"suffix",this.downloadDataSetName[0].suffix)
                        this.$set(curEvent,"url",this.downloadDataSetName[0].address)
                        this.$set(curEvent,"visual",this.downloadDataSetName[0].visual)
                        this.$set(curEvent,"value",this.downloadDataSetName[0].address)
                    }
                    ...
                    for (j = 0; j < events.length; j++) {
                        ...
                        if (events[j].eventType == "response") {
                            ...
                            if (children === undefined) {
                                ...
                                let hasFile = false;
                                for (k = 0; k < children.length; k++) {
                                        if (children[k].value != undefined && children[k].value.toString().trim() != "") {
                                        hasFile = true;
                                        break;
                                    }
                                }
        ...
        async loadTask(){
            ...
            try{
                res = await (
                    await fetch("/task/TaskInit/" + id).catch(
                    ()=>{
                    ...
                    if (state.event[j].data != undefined && state.event[j].eventType == "response") {
                        ...
                        if (nodes != undefined && refName != "grid" && refName != "table" && refName != "shapes") {
                            ...
                            for (k = 0; k < nodes.length; k++) {
                                ...
                                let node = nodes[k];
                                let configInfo = this.findConfigNode(state.event[j].data[0].text,node.text)
                                //普通输入
                                if(configInfo.component != 'file')
                                {

                                    let child = {};
                                    child.eventId = state.event[j].eventId;
                                    if(state.event[j].eventName == node.text)
                                        child.eventName = node.text+' ';
                                    else
                                        child.eventName = node.text;

                                    child.eventDesc = node.desc;
                                    child.dataType = node.dataType;
                                    child.optional = state.event[j].optional;
                                    child.configInfo = configInfo;
                                    child.value = child.configInfo.default
                                    child.child = true;
                                    children.push(child);
                                ...
                                {
                                //    文件
                                    state.event[j].configInfo = configInfo;
                                    state.event[j].dataType = configInfo.component;
                                    state.event[j].child = false;
