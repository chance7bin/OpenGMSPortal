...
Format.prototype.refresh = function()
{
	...
	else
	{
         ...
         * 张硕
         * 2019.11.12
         * 根据全局变量editModel来判断右侧格式面板是否需要 style text model 面板
         */
        // ModelInfo
        mxUtils.write(label4, mxResources.get('model'));
        div.appendChild(label4);

        var modelPanel = div.cloneNode(false);
        modelPanel.style.display = 'none';
        this.panels.push(new ModelPanel(this, ui, modelPanel));
        this.container.appendChild(modelPanel);
 ...
 * 张硕
 * 2019.11.12
 * 为适应editModel，新增 Model Desripition面板
 */
ModelPanel = function(format,editorUi,container){
    BaseFormatPanel.call(this, format, editorUi, container);
    this.init();
};

mxUtils.extend(ModelPanel, BaseFormatPanel);
...
ModelPanel.prototype.init = function()
{
    var graph = this.editorUi.editor.graph;
    var ss = this.format.getSelectionState();
    this.container.appendChild(this.addProcessDescription(this.createPanel()));
    this.container.appendChild(this.addProcessLink(this.createPanel()));
};

/**
 * Adds the process description to the given menu
 */
ModelPanel.prototype.addProcessDescription = function(div)
{
    var ui = this.editorUi;
    var editor = ui.editor;
    var graph = editor.graph;

    var title = this.createTitle("Process Description : ");
    ...
    text.onblur = function(){
        ...
        if (value === null || typeof(value) === "string")
        {
            ...
            var content = document.getElementById('process_description').value;
...
/**
 * Adds the process link to the given menu
 */
ModelPanel.prototype.addProcessLink = function(div)
{
    var ui = this.editorUi;
    var editor = ui.editor;
    var graph = editor.graph;

    var title = this.createTitle("Process Link : ");
    title.style.paddingBottom = '6px';
    title.style.fontSize = "14px";
    title.style.cursor = "default";
    div.appendChild(title);
    ...
    text.onblur = function(){
        ...
        if (value === null || typeof(value) === "string")
        {
            ...
            var obj = doc.createElement('object');
            obj.setAttribute('label', value);
            var content = document.getElementById('process_link').value;
            obj.setAttribute("relate_link", content);
        ...
        {
            var content = document.getElementById('process_link').value;
            value.setAttribute("relate_link", content);
            graph.getModel().setValue(cell, value);
        }
    };
