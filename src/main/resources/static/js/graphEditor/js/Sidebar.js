...
Sidebar.prototype.init = function () {
   ...
   * 张硕
   * 2019.11.15
   * 如果不是Create Logical Model，就将左侧面板分为两个tab；如果是，就不需要
   */
  ...
  if (typeof createLogical == "undefined" || createLogical == false) {
    ...
    generalTab.onclick = function () {
      ...
      document.getElementById("geoicon").style.display = "none";
      document.getElementById("geoicon_tab").style.backgroundColor = "#f1f3f4";
      ...
      document.getElementById("geoicon_tab").style.borderBottom = "1px solid #dadce0";
    };

    var geoIconTab = document.createElement("div");
    geoIconTab.style = "border-width: 0px 1px 1px 0px;border-style:solid;border-color:#dadce0; background-color: #f1f3f4;text-align: center; font-weight: bold;font-color:#707070;font-size: 16px; overflow: hidden; display: inline-block; padding-top: 8px; height: 25px; width: 49%;";
    geoIconTab.textContent = "GeoIcon";
    geoIconTab.id = "geoicon_tab";
    geoIconTab.onclick = function () {
      document.getElementById("geoicon").style.display = "block";
      document.getElementById("general").style.display = "none";
      document.getElementById("general_tab").style.backgroundColor = "#f1f3f4";
      document.getElementById("geoicon_tab").style.backgroundColor = "#ffffff";
      document.getElementById("geoicon_tab").style.borderBottom = "0px";
      document.getElementById("general_tab").style.borderBottom = "1px solid #dadce0";
    };

    this.sideHeaderContainer.appendChild(geoIconTab);
    this.sideHeaderContainer.appendChild(generalTab);

    this.sideGeoIconContainer = document.createElement("div");
    this.sideGeoIconContainer.style.height = "100%";
    this.sideGeoIconContainer.style.display = "none";
    this.sideGeoIconContainer.id = "geoicon";
    this.container.appendChild(this.sideGeoIconContainer);
    ...
    // 加载 geoicon_tab 页面的图形
    this.addGeoIconPalette();

  }
  else {
    var generalTab = document.createElement("div");
    generalTab.style = "border-width: 0px 0px 1px 0px;border-style:solid;border-color:#dadce0; text-align: center;font-color:#707070; font-weight: bold;font-size: 16px; overflow: hidden; display: inline-block; padding-top: 8px; height: 25px; width: 100%;";
    generalTab.textContent = "General";
    generalTab.id = "general_tab";
 ...
 */
Sidebar.prototype.addPaletteFunctions = function (id, title, expanded, fns, main_content) {
  this.addPalette(id, title, expanded, mxUtils.bind(this, function (content) {
    for (var i = 0; i < fns.length; i++) {
      var item = fns[i](content);
 ...
 * Adds the given palette.
 */
Sidebar.prototype.addPalette = function (id, title, expanded, onInit, main_content) {
  if (main_content == undefined)
    main_content = this.sideGeneralContainer;
  var elt = this.createTitle(title);
  main_content.appendChild(elt);
  ...
  var outer = document.createElement('div');
  outer.appendChild(div);
  main_content.appendChild(outer);
    ...
    success: (result) => {
      ...
      for (var i = 0; i < parentIds.length; i++) {
        $.ajax({
          ...
          type: 'get',
          async: false,
          data: {"uid": parentIds[i].id},
          success: (result) => {
            var icons = result.data;
            var geoIcon = icons.geoIcons;
            var fns = [];
            for (var j = 0; j < icons.count; j++) {
              fns.push(this.createVertexTemplateEntry('image;html=1;labelBackgroundColor=#ffffff;image=' + prefix + geoIcon[j].icon_id,
                  this.defaultImageWidth, this.defaultImageHeight, '', geoIcon[j].icon_name, false, null, null));
            }
            this.addPaletteFunctions(parentIds[i].name, parentIds[i].name, false, fns, this.sideGeoIconContainer);
          }
        });
      }
    }
