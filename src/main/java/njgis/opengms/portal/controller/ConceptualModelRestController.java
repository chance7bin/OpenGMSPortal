...
public class ConceptualModelRestController {
    ...
    public JsonResult updateConcept(MultipartFile info, HttpServletRequest request) throws IOException {
        MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) request;
        List<MultipartFile> files=multipartRequest.getFiles("imgFiles");
        MultipartFile file=multipartRequest.getFile("conceptualModel");
        String model=IOUtils.toString(file.getInputStream(),"utf-8");
    ...
    public JsonResult updateKnowledge(KnowledgeDTO knowledgeDTO, HttpServletRequest request) throws IOException {
        ...
        JSONObject result = repositoryService.updateKnowledge(ItemTypeEnum.ConceptualModel ,knowledgeDTO,email);
